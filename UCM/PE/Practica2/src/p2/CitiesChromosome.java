...
public class CitiesChromosome extends Chromosome<Integer> {
	...
	public CitiesChromosome(Function f) {
		...
		//A�adimos madrid al comienzo del cromosoma
		genes.add(25);
		//A�adimos el resto de ciudades al cromosoma
		int min = Math.min(number_arguments, 25);

		for (int i = 0; i < min; i++) { genes.add(i); }
		//A�adimos madrid al final del cromosoma
		for (int i = min + 1; i < number_arguments; i++) { genes.add(i); }
		genes.add(25);

		//A�adimos madrid al final
		...
		if(Math.random() < prob) {
			switch(mutation) {
				case INSERTION:
					...
					// Algoritmos
					// 1. Elegimos posici�n aleatoria para elegirlo como desplazante  (1 - 26)
					int moving_pointer = randomRange(1, 26);
					// 2. Elegimos segunda posici�n aleatoria para desplazar.
					...
					// 3. Desplazamos todo el subarray hacia la derecha (1 posici�n)
				...
				case SELF_METHOD_1:
					...
					// Miramos si haciendo swap a la derecha o a la izquierda, se mejora la evaluaci�n, si es as�, cambiamos.
	...
	public void cross(@SuppressWarnings("rawtypes") Chromosome chr1, CrossType type) {
		...
		switch(type) {
			case PARTIALLY_MAPPED: // PMX
				...
				// Vemos 0 - (min - 1)
				for(int i = 0; i < min; ++i) {
					// Vemos si est� dentro del subconjunto.
