...
Cada algoritmo/estructura se implementa en una función contar_frecuencias_Vi que se invoca desde el main.
 ...
 - Se incluye la función contar_frecuencias_V5 que hace uso de un TDA DispersedVector (vector disperso).
    Prueba a ejecutar esta función cuando termines la implementación del TDA DispersedVector
*/
...
/* Metodo que permite contar las frecuencia de aparicion de una palabra en un texto
  ...
  fin: limite superior del libro hasta la que queremos contar (libro[ini] hasta libro[fin-1])
  pal: vector de palabras ya leidas
  frec: DispersedVector que indica el numero de veces que ocurre la palabra pal[i] en el libro (desde libro[ini] hasta libro[fin-1])

  Notas: - este código reutiliza el algoritmo implementado en contar_frecuencias_V3, pero utilizando un vector disperso (DispersedVector) para las frecuencias.
         - reflexiona sobre cómo hacer más competitivo este código con el TDA DispersedVector en términos de eficiencia
*/

void contar_frecuencias_V5( vector<string> & libro, int ini, int fin,
                         vector<string> &pal, DispersedVector<int> & frec ){
...
int main() {
 ...
 ////////////////////////////////////////////////////
 // CONTAR FRECUENCIAS V5  --- Con TDA. Vector Disperso (DispersedVector)
 ////////////////////////////////////////////////////
 DispersedVector<int> frecuencia_disperso(Dicc.size(), 0);
