...
Action ComportamientoJugador::think(Sensores sensores){
    ...
    // A* Origen
    if(plan.empty() || sensores.reset || (destino.fila != sensores.destinoF && destino.fila != sensores.destinoC)){
        estado origen;
        origen.fila = fil;
        origen.columna = col;
    ...
    */
    ...
    //mapaResultado[fil + dx][col] = sensores.terreno[0];
    /*for(int j = 0; j < 3; ++j){
        int m = (j * 2 + 1);
        for(int i = -j; i <= j; ++i){
            mapaResultado[i][j] = sensores.terreno[++p];
        }
    }*/
    ...
    if(!plan.empty()){
        ...
        if(sensores.terreno[0] != 'a'){
            ...
            cout << "ALDEANO EN MI CAMINO" << endl;
            movimiento = actIDLE;
        }
    }else{
        cout << "NO HAY PLAN" << endl;
        int total = 0;
        if(puede_pasar(sensores.terreno[0])){
            for(int i = 0; i < 9; ++i){
                total += ~puede_pasar(sensores.terreno[i]);
            }
        }else{
            movimiento = actTURN_R;
        }
        float pb = (total + aleatorio(3)) / 12.0;
        if(pb >= 6.0){
            movimiento = actTURN_R;
        }else{
